Attribute VB_Name = "LibraryQC"
Option Explicit

Sub TestCompareSheets()
    Call CompareSheets("MA", "ABU MA", "QC MA")
End Sub

Sub CompareSheets(sSheet1 As String, sSheet2 As String, sQCSheet As String)
    Dim oSheet1 As Worksheet, oSheet2 As Worksheet, oQCSheet As Worksheet
    Dim iMaxRow As Long, iMaxColumn As Long
    
    Set oSheet1 = FindSheet(sSheet1)
    Set oSheet2 = FindSheet(sSheet2)
    Set oQCSheet = FindSheet(sQCSheet)
    
    If oQCSheet Is Nothing Then
        Set oQCSheet = Add_Sheet(sQCSheet, oSheet2.Index)
    End If
